...
def printAtConsole(header, info, solution=""):
    global console
    switch = {
        "fex": "FUNCTION EXIT", "FEX": "FUNCTION EXIT",
        "fen": "FUNCTION ENTRY", "FEN": "FUNCTION ENTRY",
        "tas": "TASK", "TAS": "TASK",
        "err": "ERROR", "ERR": "ERROR",
        "war": "WARNING", "WAR": "WARNING",
    }
...
def getImage(cropbtn):
    printAtConsole("fen", "On: {} ;By: {}".format(inspect.stack()[0][3], inspect.stack()[1][3]))
    os.chdir(os.getcwd())
    acquire_image_wia()
    global scanned
    if scanned == 1:
    ...
    printAtConsole("fex", "On: {} ;By: {}".format(inspect.stack()[0][3], inspect.stack()[1][3]))
...
def acquire_image_wia():
    ...
    try:
        ...
        printAtConsole("err", "Device Error", solution="Switch on the printer and connect to the computer]")
    i = 1
...
scan_crop.grid(column=0, row=2, columnspan=2, pady=C4U.grid_offsets['y1'], padx=C4U.grid_offsets['x1'])
Separator(window).grid(column=0, row=3, sticky="ew")
console = ScrollableFrame(window)
console.grid(column=0, row=4, columnspan=2, pady=C4U.grid_offsets['y1'], padx=C4U.grid_offsets['x1'])
...
imgbtn.grid(ipady=5, ipadx=10, column=0, row=0, pady=C4U.grid_offsets['y2'], padx=C4U.grid_offsets['x2'])
cropbtn = Button(scan_crop,state='disabled', text="Crop",
                 command=lambda: doTheJob(do))
cropbtn.grid(ipady=5, ipadx=10, column=0, row=1, pady=C4U.grid_offsets['y2'], padx=C4U.grid_offsets['x2'])
do = Button(scan_crop,state='disabled', text="Get Print",
            command=lambda: addAddress())
do.grid(ipady=5, ipadx=10, column=0, row=2, pady=C4U.grid_offsets['y2'], padx=C4U.grid_offsets['x2'])
# ----------------
# Console
# ----------------
# # Added at runtime
